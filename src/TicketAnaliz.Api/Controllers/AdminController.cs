...
public class AdminController : ControllerBase
{
    ...
    public async Task<ActionResult<SuggestionLogDetailDto>> GetHistoryDetail(Guid id, CancellationToken ct)
    {
        ...
        var detail = new SuggestionLogDetailDto
        {
            ...
            WebSources = JsonSerializer.Deserialize<List<WebSourceDto>>(log.WebSourcesJson, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }) ?? new List<WebSourceDto>(),
            WebSearchQuery = log.WebSearchQuery,
            WebSearchResultCount = log.WebSearchResultCount,
            WebSearchAcceptedCount = log.WebSearchAcceptedCount,
            WebSearchDurationMs = log.WebSearchDurationMs,
